...
<div>
  ...
  <input type="date" name="fecha" value="{{ old('fecha', $misa->fecha ?? '') }}" class="w-full border px-3 py-2 rounded" required>
</div>

<div>
  <label class="block text-sm font-semibold">Hora</label>
  <input type="time" name="hora" value="{{ old('hora', $misa->hora ?? '') }}" class="w-full border px-3 py-2 rounded" required>
</div>

<div>
  <label class="block text-sm font-semibold">Tipo de Misa</label>
  <select name="tipo_misa" id="tipo_misa" class="w-full border px-3 py-2 rounded" required onchange="asignarEstipendio()">
...
<div>
  ...
  <select name="id_sacerdote" class="w-full border px-3 py-2 rounded">
...
<div>
  ...
  <textarea name="intencion" class="w-full border px-3 py-2 rounded">{{ old('intencion', $misa->intencion ?? '') }}</textarea>
</div>

<div>
  <label class="block text-sm font-semibold">Observaciones</label>
  <textarea name="observaciones" class="w-full border px-3 py-2 rounded">{{ old('observaciones', $misa->observaciones ?? '') }}</textarea>
</div>

<div>
  <label class="block text-sm font-semibold">Estipendio (Bs.)</label>
  <input type="text" id="estipendio" name="estipendio" value="{{ old('estipendio', $misa->estipendio ?? '') }}" class="w-full border px-3 py-2 rounded bg-gray-100" readonly>
</div>

<div>
  <label class="block text-sm font-semibold">Estado</label>
  <select name="estado" class="w-full border px-3 py-2 rounded" required>
...
<script>
  function asignarEstipendio() {
    ...
    const estipendio = document.getElementById('estipendio');
    estipendio.value = precios[tipo] ?? 0;
  }
