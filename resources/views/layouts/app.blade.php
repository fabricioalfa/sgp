...
<head>
  ...
  <style>
    ...
    table td { color: #573830 !important; }
  </style>
</head>
  ...
  <aside class="w-64 bg-[#E9A209] text-white shadow-lg flex flex-col h-full text-lg backdrop-blur-sm/40">
    ...
    <nav class="flex-1 px-4 py-6 space-y-3 font-semibold overflow-y-auto">
      ...
      @if($user && $user->rol === 'administrador')
        ...
        <a href="{{ route('reportes.index') }}" class="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-[#C1440E] transition">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path d="M3 3h18v18H3z"/><path d="M7 11h10M7 15h10M7 7h5"/>
          </svg>
          Reportes
        </a>
